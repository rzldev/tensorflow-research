## Building Model 3: GRU Model (RNN) ##
...
def run():
    ...
    '''
    NOTE:
    Another popular and effective RNN component is the GRU (Gated Recurrent Unit). The
    GRU layer has similar features to an LSTM layer but has less parameters.
    '''
    ...
    model_3_tensorboard_cb = utils.create_tensorboard_callback(dir_name=SAVE_DIR, 
                                                               experiment_name=model_3.name)
    model_3_history = model_3.fit(training_texts, training_labels,
                                  epochs=5,
                                  validation_data=(validation_texts_20_percent,
                                                   validation_labels_20_percent),
                                  callbacks=[model_3_tensorboard_cb])
    
    ## Evaluating the model
    print(f'\nEvaluating {model_3.name}')
    model_3.evaluate(validation_texts, validation_labels)
    utils.plot_model_history(model_3_history)
    
    #$ Making predictions & compare the results
    print(f'\nMaking predictions using {model_3.name}')
    model_3_preds = model_3.predict(validation_texts)
    model_3_preds = tf.squeeze(tf.round(model_3_preds))
    model_3_results = utils.evaluate_model_results(validation_labels, model_3_preds)
    print(f'\nModel 3 (GRU) results: \n{model_3_results}')
    utils.plot_confusion_matrix(validation_labels, model_3_preds)
    
    return model_3_history, model_3_results
