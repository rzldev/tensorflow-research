## Building Model 4: Bidirectional-LSTM Model (RNN) ##
...
def run():
    ...
    ## Create the Model 4: Bidirectional-LSTM
    ...
    '''
    NOTE:
    Normal RNN's go from left to right, however a bidirectional RNN goes from right to
    left as well as left to right
    '''
    ...
    model_4 = Model(inputs, outputs, name='model_4_bidirectional_lstm')
    ...
    model_4_tensorboard_cb = utils.create_tensorboard_callback(dir_name=SAVE_DIR, 
                                                               experiment_name=model_4.name)
    model_4_history = model_4.fit(training_texts, training_labels,
                                  epochs=5,
                                  validation_data=(validation_texts_20_percent,
                                                   validation_labels_20_percent),
                                  callbacks=[model_4_tensorboard_cb])
    
    ## Evaluating the model
    print(f'\nEvaluating {model_4.name}')
    model_4.evaluate(validation_texts, validation_labels)
    utils.plot_model_history(model_4_history)
    
    ## Making predictions
    print(f'\nMaking predictions using {model_4.name}')
    model_4_preds = model_4.predict(validation_texts)
    model_4_preds = tf.squeeze(tf.round(model_4_preds))
    model_4_results = utils.evaluate_model_results(validation_labels, model_4_preds)
    print(f'\nModel 4 (Bidirectional-LSTM) results: \n{model_4_results}')
    utils.plot_confusion_matrix(validation_labels, model_4_preds)
    
    return model_4_history, model_4_results
