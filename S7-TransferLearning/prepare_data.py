...
def prepare(path, output_path, datasets=['train', 'test'], labels=['pizza', 'steak'],
            meta_dir='meta', train_data_size=1):
    '''
    Preparing dataset directory to use to train CNN model
    '''
    print('\nPreparing data.')
    for dataset in datasets:
        copy_images(path, output_path, dataset, meta_dir, labels, train_data_size)
...
def copy_images(path, output_path, dataset, meta_dir, target_labels, train_data_size):
    '''
    ...
    if dataset == 'train' or dataset == 'training':
        for label in labels:
            data = labels[label]
            labels[label] = rand.sample(data, int(len(data) * train_data_size))
    ...
    for label in target_labels:
        ...
        if len(os.listdir(f'{output_path}/{dataset}/{label}')) >= len(labels[label]):
            continue
